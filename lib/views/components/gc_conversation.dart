    ...
    return FirestorePagination(
      ...
      itemBuilder: (context, docs, index) {
        ...
        return GCMessagTile(
          ...
          sender: data["sender"],
          sentByLocalUser: (data["sender"].toString() == Constants.localUsername) ? true : false,
          time: DateTime.fromMillisecondsSinceEpoch(int.parse((data["time"]).toString())),
  ...
  Widget groupInfo() {
    return Container(
      decoration: BoxDecoration(
          color: HexColor("#262630"), borderRadius: const BorderRadius.vertical(top: Radius.circular(20))),
      child: Column(
        children: [
          ...
          Container(
            ...
            alignment: Alignment.centerLeft,
            child: Text(
              "At ${DateFormat('yyyy-MM-dd, kk:mm').format(DateTime.fromMillisecondsSinceEpoch(widget.createdAt))}",
              style: GoogleFonts.archivo(color: Colors.white, fontWeight: FontWeight.w100, fontSize: 12),
            ),
          ),
          ...
          Container(
            ...
            child: Row(
              children: [
                RandomAvatar((widget.data)[widget.createdBy], height: 30),
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                return Container(
                  ...
                  child: Row(
                    children: [
                      RandomAvatar(((widget.data).values).toList()[index], height: 30),
                      // const SizedBox(width: 5),
                      Container(
                        padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 10),
                        child: Text(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: GestureDetector(
            onTap: () => Navigator.pushReplacement(
                context, MaterialPageRoute(builder: (context) => const ChatRoom(userType: UserType.student))),
      ...
      body: Container(
        ...
        child: Stack(
          children: [
            SizedBox(height: MediaQuery.of(context).size.height / 1.5, child: paginate()),
            // const SizedBox(height: 10),
            Container(
...
class _GCMessagTileState extends State<GCMessagTile> {
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        if (!widget.sentByLocalUser)
          Column(
            children: [
              Container(
                margin: const EdgeInsets.only(right: 10),
                child: RandomAvatar(widget.userSvg, height: 30),
              ),
              const SizedBox(height: 10)
            ],
          ),
        Column(
          ...
          children: [
                      ...
                      child: Align(
                        ...
                        child: Container(
                          constraints: BoxConstraints(
                            maxWidth: MediaQuery.of(context).size.width * 0.60, // 90% of screen width
                          ),
                          ...
                          child: Column(
                            crossAxisAlignment:
                                widget.sentByLocalUser ? CrossAxisAlignment.start : CrossAxisAlignment.end,
                            children: [
                              widget.deleted
                                  ? Row(
                                      children: [
            ...
            Align(
              alignment: widget.sentByLocalUser ? Alignment.centerRight : Alignment.centerLeft,
              child: Text(
                widget.sentByLocalUser
                    ? "${DateFormat("HH:mm").format(widget.time)} · ${widget.sender}"
                    : "${widget.sender} · ${DateFormat("HH:mm").format(widget.time)}",
