...
class DatabaseMethods {
  ...
  Future<QuerySnapshot?> searchUsersByEmail(String email, String collectionName) async {
    return await FirebaseFirestore.instance.collection(collectionName).where('email', isEqualTo: email).get();
  }
  ...
  Future<void> generateSearchKeywords(String userId, String username) async {
    ...
    Stream<Map<String, dynamic>> getLastMessage(String roomId) {
      return FirebaseFirestore.instance.collection('chatRooms').doc(roomId).snapshots().map((snapshot) {
        return {
  ...
  updateUnreadMessages(String chatId, int messages, String receiver) async {
    return await FirebaseFirestore.instance
        .collection("chatrooms")
        .doc(chatId)
        .update({"unreadMessages.$receiver": messages});
  }
...
class FirestoreService {
  ...
  Future<void> sendMessage(String conversationId, Message message) async {
    await _firestore.collection('conversations').doc(conversationId).collection('messages').add(message.toMap());
